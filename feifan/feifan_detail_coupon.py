...
def get_detail(thread_name, data_list):
    ...
    base_url = "http://h5.ffan.com/app/coupon?cid=%s&cityId=%s&plazaId=%s&display_type=html"
    ...
    for index, data_bean in enumerate(data_list):
        ...
        while True:
            try:
                ...
                if len(fc_apply_store) | len(fc_more_details) | len(fc_limit):
                    ...
                    try:
                        ...
                        update_sql = base_update_sql % {
                            'fc_apply_store': fc_apply_store_json,
                            'fc_more_details': fc_more_details,
                            'fc_limit': fc_limit,
                ...
                retry_count += 1
    db.close()
    print("coupon detail operate db threadName : %s 耗时：%s 秒" % (thread_name, (util.time_utils.get_current_time() - start_time)))
